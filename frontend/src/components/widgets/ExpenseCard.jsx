...
import { Globe, ShoppingCart, RotateCcw, SquarePen, Trash } from "lucide-react";
...
export const ExpenseCard = ({ expense, toggleDetails }) => {
  const { toggleModal, isOpenDeleteModal, toggleDeleteModal } = useExpenseStore(
    (state) => ({
  ...
  const { mutateAsync: restoreExpense, isPending: isRestoringExpense } =
    useRestoreExpenseFromTrash();
  ...
  const handleRestoreItem = async () => {
    try {
      const res = await restoreExpense({
        trashedItem: expense,
        owner: user?.accountId,
      });

      // Handle response format: { success, message, data } or { ok: true }
      if (res?.success || res?.ok) {
        showToast(res?.message || "Expense restored successfully.", "success");
      } else {
        showToast(res?.message || "Failed to restore expense.", "error");
      }
    } catch (error) {
  ...
  return (
    <li className="my-4 bg-white rounded-lg shadow-lg p-6 dark:bg-dark-secondary">
      ...
      <p className="text-gray-600 mt-2 mb-4 dark:text-gray-400 line-clamp-2">
        {expense.description}
      </p>
        ...
        <div className={isRestoringExpense && "pointer-events-none"}>
          {expense?.isTrashed ? (
            <button
              ...
              className="bg-gradient-blue text-white p-1 rounded mr-2"
            >
              <span>
                <RotateCcw size={20} />
              </span>
            ...
            <button
              onClick={() => toggleModal("edit", expense)}
              className="bg-gradient-blue text-white p-1 rounded mr-2"
            >
              <span>
                <SquarePen size={20} />
          ...
          <button
            onClick={() => toggleDeleteModal(expense)}
      ...
      <button
        className="text-blue-500 font-light border-[.5px] border-blue-500 p-2 w-full"
