...
export const DeleteAlertModal = ({ permanent }) => {
  ...
  const { mutateAsync: moveToTrashExpense, isPending: isMovingToTrash } =
    useMoveToTrashExpense();
  ...
  return (
    <div
      className={`fixed top-0 left-0 z-50 w-full h-full bg-black/30 backdrop-blur flex justify-center items-center px-6 py-10`}
    >
      <div className="w-full bg-white rounded-md px-4 py-6 overflow-auto md:max-w-3xl dark-theme">
        ...
        <p className="body-text">
          ...
          {permanent ? (
            <p className="bg-orange-100 text-orange-700 px-2 py-4 text-sm rounded my-2 flex items-center gap-2">
              ...
              <span>
                Once you delete it, it's gone forever. No way to bring it back!
              </span>
            </p>
          ) : (
            <p className="bg-purple-100 text-purple-700  px-2 py-4 text-sm rounded my-2 flex items-center gap-2">
              ...
              <span>
                This will be moved to trash. Don't worry, you can restore it
                anytime from the trash.
              </span>
            </p>
          )}
        </p>
        <div
          className={`mt-4 flex justify-end ${
            (isMovingToTrash || isDeleting) && "pointer-events-none"
          }`}
        >
          <button
            disabled={isDeleting}
