...
import { showToast, formatDateForInput } from "@/lib/utils";
...
export const ExpenseModal = () => {
  ...
  const { isOpenModal, actionType, toggleModal, editableItem } =
    useExpenseStore();
  ...
  const onSubmit = async (data) => {
    data.owner = user?.accountId; // Use accountId, not $id
    ...
    try {
      ...
      if (actionType === "add") {
        ...
        res = await updateExpense({
          ...
          owner: user?.accountId, // Use accountId, not $id
        });
      }
      ...
      if (res?.success) {
        ...
        showToast(
          res?.message ||
            `Failed to ${actionType === "add" ? "create" : "update"} expense.`,
          "error"
        );
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (actionType === "edit" && editableItem) {
      const newDate = formatDateForInput(editableItem.date);
      ...
      <div className="h-full w-full bg-white rounded-md p-4 overflow-auto md:max-w-3xl md:h-auto dark-theme">
        <div className="flex justify-between items-center">
          <h2 className="heading3">
            {actionType === "edit" ? "Edit" : "Add new"} expense
          </h2>
          <button
            onClick={toggleModal}
            className="bg-gray-50 shadow p-2 rounded text-gray-600"
            type="button"
          >
            <X />
          </button>
        ...
        <form className="py-6" onSubmit={handleSubmit(onSubmit)}>
          ...
          >
            {isCreatingExpense ||
              (isUpdatingExpense && <Loader color="white" />)}
