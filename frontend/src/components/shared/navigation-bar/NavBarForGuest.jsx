          ...
          <div className="flex h-full w-full flex-between">
            ...
            <div className="flex gap-2 items-center md:hidden">
              <button className="" type="button">
                <Link
                  to="https://github.com/fazle-rabbi-dev/RupyaTrack"
                  target="_blank"
                >
                  <Github size={20} />
                </Link>
    ...
    <nav className="absolute top-0 right-0 h-screen w-full bg-black/30 backdrop-blur flex justify-end">
      <div className="bg-light-50 shadow-md h-screen w-[80vw] py-5 px-6 dark-theme">
        <div className="flex justify-between items-center">
          <Logo />
          <button
            className="bg-gray-100 text-gray-600 p-2 rounded"
            onClick={toggleNav}
            type="button"
          >
            <X />
            ...
            <li onClick={toggleNav} key={link.id}>
              <Link
                className={`block py-1 ${
                  link.href === pathname && "text-gradient-blue-sky font-bold"
                }`}
                to={link.href}
              >
                {link.name}
  ...
  return (
    <nav className="hidden md:block">
      <ul className="flex gap-4 items-center">
        {NavLinksForGuest?.map(
          (link) =>
            link.name !== "Download Apk" && (
              <li key={link.id}>
                <Link
                  className={
                    link.href === pathname && "text-blue-600 font-bold"
                  }
                  to={link.href}
                >
                  {link.name}
