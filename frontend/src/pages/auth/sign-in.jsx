...
export const SignInPage = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (!res?.success) {
        showToast(res?.message || "Login failed.", "error", 2000);
        return;
      }

      // Success: update auth state and redirect
      await checkAuth();
      showToast(res.message || "Logged in successfully.", "success", 2000);
      navigate("/dashboard", { replace: true });
    } catch (error) {
      showToast("Login failed. Please try again.", "error", 2000);
    }
  };

  const authenticateUser = async (token) => {
    try {
      const res = await saveUserAfterSocialAuth(token);
    ...
    if (location.search) {
      ...
      if (params.get("authstatus") === "success") {
        ...
        if (token) {
          ...
          showToast("Authentication failed - no token received.", "error", 2000);
          navigate("/sign-in", { replace: true });
        }
      }
      if (params.get("authstatus") === "failed") {
        showToast("OAuth authentication failed.", "error", 2000);
        navigate("/sign-in", { replace: true });
      }
    }
  }, [location]);
    ...
    <section className="flex flex-1 flex-col items-center justify-center">
      <div className="w-full">
        <div className="bg-white shadow rounded-md px-8 pt-6 pb-8 md:bg-transparent md:shadow-none dark-theme">
          <h1 className="heading3 text-gray-800 mb-2 dark:text-white">
            {"🔐"} Sign In
          </h1>
          ...
          <div className="flex items-center justify-center mb-4">
            ...
            <span className="text-sm text-gray-600 mx-4 dark:text-gray-300">
              OR
            </span>
            <hr className="flex-grow border-gray-200" />
          </div>
